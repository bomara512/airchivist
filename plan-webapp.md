...
**Left as literal, not tokenized**: the handful of icon-glyph sizes (`.theme-toggle-btn` and `.shelf-icon-btn` at `1rem`, `.suggestion-dismiss-btn` at `1.2rem`, `.shelf-header h2::before`'s caret at `1.3em`, `.carousel-arrow` at `1.4rem`) — these size a single Unicode character used as a button icon, not reading text, so folding them into the type scale risked visually breaking icon proportions for no consistency benefit (nothing else shares their role). The two intentional `font-family: monospace` uses (`.alias-pill`, `.alias-edit-input`, for displaying tag data) are unrelated to this pass and were left untouched — their *font-size* did get pulled into `--font-size-sm` like any other 0.8rem text, just not their font-family.
