...
**Grouping**: The group select offers "No grouping" (default) and "By channel". When "By channel" is selected, the route partitions the current page's videos by `channel_name` in Python, producing one section per channel in the order they first appear. The result is a list of `{"tag": {"name": channel_name}, "videos": [...]}` dicts, reusing the same structure `_video_container.html` already renders for any grouped view.
