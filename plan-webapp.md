...
- `GET /api/channel/status?url=<channelUrl>` — validates `url` against `YT_CHANNEL_RE` (400 `{"status": "error", "error": "Not a YouTube channel URL"}` if it doesn't match), then looks up via `get_channel_by_source_url`. Returns `{"status": "exists", "channel_name": ...}` if found, else `{"status": "not_found"}`.
...
`webapp/static/style.css` opens with a `:root` block of CSS custom properties (`--bg`, `--bg-elevated`, `--bg-recessed`, `--border`, `--border-input`, `--text`, `--text-muted`, `--text-subtle`, `--hover-border`, `--pill-bg`, `--pill-text`, `--thumb-placeholder`, `--accent`, `--accent-hover`, `--link-hover`, `--card-shadow`) holding the dark-theme values — these are the app's original, unchanged colors. A light palette overrides the same tokens twice: once inside `@media (prefers-color-scheme: light) { :root:not([data-theme="dark"]) { ... } }` (applies only when the OS prefers light AND the user hasn't explicitly forced dark), and once in `:root[data-theme="light"] { ... }` (forces light regardless of OS). No `data-theme` attribute present = "System" — the media query alone decides.

The toggle lives in `base.html`'s header nav as `#theme-toggle-btn` (`.theme-toggle-btn`, an icon-only button styled to match the toolbar's other bordered controls). Its script cycles System → Light → Dark → System on click, storing the explicit choice (`"light"` or `"dark"`) in `localStorage['theme']` or removing that key entirely for "System". The icon (◐ / ☀ / ☾) and `title` tooltip reflect the current *setting*, not necessarily the rendered appearance. A second, separate inline `<script>` sits in `<head>` — before the stylesheet `<link>`, and deliberately *not* `defer`red like the app's other scripts — that reads `localStorage` and sets `data-theme` on `<html>` synchronously before first paint; without it, a user who chose "Light" would see a flash of the dark theme on every full page load, since the preference lives client-side only and the server has no way to know it when rendering the initial HTML.

**Scope of the color-token conversion**: only colors that recur across the whole app (page/card/input backgrounds, the 4-step text-gray hierarchy, borders, tag/pool pills, thumbnail placeholders, and the two link-hover reds) were converted to tokens — roughly a dozen distinct values that, because they're reused identically everywhere, end up giving every page (main list, tags admin, channels, watch-later, hidden, modals) a working light theme even though only the main list/header/video-card area was explicitly mocked up and approved before implementing. Deliberately left as literal, theme-invariant colors: the accent red button background/hover (`#cc0000`/`#ee0000` — already has enough contrast on both a near-black and a white background, so no light-mode variant is needed), white text-on-colored-background (`#fff` on red buttons or black thumbnail overlays), the black `rgba(0,0,0,…)` thumbnail/modal overlays (sit on photos or a fixed backdrop, not the page background), and every one-off status/decorative color — the LLM tag-suggestion panel's blue/green/amber palette, confidence badges, the favorite-star gold (`#f5c518`), watched/applied green (`#4caf50`), `nav-hidden` orange, and the tags-admin page's `#111`/`#555`/`#252525`/etc. form and pill colors. These are self-contained (each sets its own background *and* foreground together) so they render fine on either theme as-is, but their contrast was not individually re-verified against a light background — a known gap, not an oversight, should any of them read poorly in light mode later.

### Type Scale (Font Size / Weight Tokens)

`:root` also defines a type scale: `--font-size-2xs` (0.7rem), `--font-size-xs` (0.75rem), `--font-size-sm` (0.85rem), `--font-size-base` (1rem), `--font-size-lg` (1.1rem), and `--font-weight-normal`/`--font-weight-semibold`/`--font-weight-bold` (400/600/700). These don't vary between light and dark, so they're defined once in the base `:root` block, not repeated in the theme overrides.

The stylesheet previously had 15 distinct font-size literals used inconsistently for the same visual role — e.g. `.video-channel` at `0.85rem` sat directly above `.video-meta` at `0.8rem` on the same card, and small buttons were variously `0.8rem`/`0.85rem`/`0.9rem` with no discernible pattern across `.reset-btn`/`.secondary-btn`/`.btn-restore`. `--font-size-sm` (0.85rem) absorbs the five near-duplicate values that were all really the same "secondary/meta text" role (`0.8`, `0.82`, `0.85`, `0.875`, `0.9rem` — about 50 of the file's ~80 font-size declarations); `--font-size-xs` absorbs `0.75`/`0.78rem`; `--font-size-base` absorbs `0.95`/`1`/`1.05rem`. The concrete bug this caught: the header nav's own secondary links weren't consistent with each other — "Tags"/"Channels"/"Watch Later" were inline-styled at `0.9rem` while "Archived"/"Install bookmarklet" used their own classes at `0.85rem`. Fixed by giving all five a shared `.nav-link-secondary` class (`webapp/templates/base.html`); `.nav-hidden`/`.nav-install` now only carry their color override.

**Left as literal, not tokenized**: the handful of icon-glyph sizes (`.theme-toggle-btn` and `.shelf-icon-btn` at `1rem`, `.suggestion-dismiss-btn` at `1.2rem`, `.shelf-header h2::before`'s caret at `1.3em`, `.carousel-arrow` at `1.4rem`) — these size a single Unicode character used as a button icon, not reading text, so folding them into the type scale risked visually breaking icon proportions for no consistency benefit (nothing else shares their role). `.alias-edit-input` (an *editable* alias-pattern text field, not a pill) intentionally keeps `font-family: monospace` — editing a literal match pattern benefits from a fixed-width font. `.alias-pill` (the read-only display pill) no longer does; see Tag Pill Consistency below.

See "Use design tokens for colors, font sizes, and font weights" in `CLAUDE.md` for the standing rule this establishes going forward.

### Tag Pill Consistency

Every "tag pill"-shaped element in the app shares the same base look — `background: var(--pill-bg); color: var(--pill-text); font-size: var(--font-size-xs)`, sans-serif — matching `.tag-pill` on the main video-card list: `.pool-tag` (unclassified-tag checkbox pills on `/tags`), `.alias-pill` (an alias pattern displayed under its canonical tag), and `.group-member-pill` (a canonical tag's membership in a named group). `.alias-pill` previously rendered in `font-family: monospace` with hardcoded (non-theme-aware) colors — dropped in favor of the shared tokens; there was no real reason for a *read-only* alias display to be monospace (its sibling *editable* field, `.alias-edit-input`, legitimately keeps monospace — see Type Scale above). A shared `--pill-hover-bg` token (dark `#333` / light `#ddd`) replaced three slightly-different literal hover backgrounds (`.alias-pill`, `.group-member-pill`, `.pool-tag` each had their own).

`.pool-tag-related` (AI-suggested "related tags" shown above the unclassified pool) is the one deliberate exception: it keeps a distinct green color scheme rather than matching the standard pill look, since it signals "this is a suggestion, not yet a real tag" — a meaningful distinction that would be lost by unifying it. It's now theme-aware via its own token family (`--pill-suggested-bg`/`-text`/`-hover-bg`/`-hover-text`/`-active-bg`, plus `--pill-suggested-container-bg`/`-border`/`-label` for the surrounding `.tag-related` box), with hand-picked light-mode greens rather than a mechanical dark-to-light inversion — there was no pre-existing light equivalent to preserve, unlike the rest of the theme which mirrors colors the app already had.
...
- **Watch status** — a single `<select name="watch_status">` with three options: `""` (default, "Include watched"), `"unwatched"` ("Unwatched only" — a true filter, maps to `unwatched_only` in the route, excludes watched videos via `_build_where`), and `"unwatched_first"` ("Unwatched first" — not a filter, reorders instead of excluding; maps to `unwatched_first`, see `unwatched_first` under `get_all_videos` above). An unrecognized value falls back to `""` (all videos) rather than a 400, matching how an unrecognized `group` value is already handled — `watch_status` never touches `_build_where`'s raw-value validation since it's decomposed into the two booleans at the route level before either DB call. Contributes at most 1 to `active_filter_count` regardless of which non-default option is chosen (`watch_status != ""`).
  ...
  - Briefly carried a `data-hint`-driven caption span next to the select explaining the current choice (to offset a plain `<select>` only showing the active label when closed), replaced the same day with self-explanatory option wording alone ("Include watched" contrasts against "Unwatched *only*" and "Unwatched *first*" without needing extra text) after considering and rejecting per-`<option>` `title` tooltips as the fix — those render unreliably across browsers for native `<select>` dropdowns, unlike `title` on ordinary buttons (e.g. the watched-toggle, shelf-refresh button) elsewhere in this app.
...
**API plumbing**: every extension-facing JSON route goes through `webapp/api.py` rather than repeating the pattern inline. `@cors_json` (directly under `@bp.route`) answers the `OPTIONS` preflight with 204, JSON-encodes the view's return value — a plain dict means 200, a `(dict, status)` tuple sets the code — and attaches `CORS_HEADERS`. `@resolve_video` (under `@cors_json`) parses the request's `url` via `crawler.models.extract_video_id`, loads the row with `get_video_by_id`, returns the shared `{"status": "error", ...}` 400/404 bodies itself, and hands the row to the view as its first argument. `ApiStatus` is a StrEnum holding every `status` value the API returns, so no route spells one as a literal. `@video_api_route` composes the two in the correct order and is what routes actually use; `@resolve_video` raises at import time if applied above `cors_json`. That guard exists because the transposed stack fails *silently*: Flask still returns the `(dict, status)` tuple, but with no CORS headers and with `OPTIONS` falling through to the view, so the test client cannot see it and only the extension breaks (verified empirically 2026-09-25). All 12 API routes go through this module as of 2026-09-25; `routes.py` carries no CORS or preflight code of its own. Five of them use `@cors_json` alone, because `@resolve_video`'s "must already exist" contract does not fit them: `api_add` creates the video, `api_status` returns `not_found` with HTTP 200 rather than a 404, `api_status_batch` takes a list of IDs instead of a URL, and the two channel routes parse channel URLs.

**Bookmarklet / quick-add**: `POST /api/add` accepts `{"url": "<youtube-url>"}` from any origin (CORS headers included). It extracts the video ID via `crawler.models.extract_video_id`, which wraps the same `YT_ID_RE` regex the crawler uses, checks for an existing `fetch_status='ok'` record, then calls `fetch_metadata(video_id, delay=0)` and persists the result via `add_video` in `webapp/db/videos.py`. Returns `{"status": "added"|"exists"|"hidden"|"error", "title": "..."}`. When the video exists but is hidden, returns `"hidden"` — the extension uses this to show the hidden-state UI without a separate status check. `add_video` uses `ON CONFLICT(video_id) DO UPDATE` but does not overwrite `date_added`, `personal_view_count`, or `date_last_viewed`. `GET /install` renders `install.html`, which shows the bookmarklet as a draggable `<a href="javascript:...">` link. The bookmarklet shows a toast notification on the YouTube page while the fetch runs, then updates it with the result. When the API returns `not_found`, the extension popup renders an "Add to Airchivist" button and two opt-in checkboxes ("Also add to Watch Later" and "Also mark as favorite (★)") instead of firing the add immediately; on button click, `doAdd` is called and, if either checkbox is ticked, parallel follow-up calls to `POST /api/watch-later/add` and/or `POST /api/favorite/add` fire after the main add (via `Promise.allSettled` — both must wait for the main add to succeed first, since they both require the video to already be in the DB, but they run in parallel with each other and neither depends on the other). `POST /api/favorite/add` returns no 409/already-case (unlike watch-later's add) since `is_favorite` is a plain boolean and `set_favorite` just force-sets it — idempotent re-adding always returns `"added"`. `POST /api/favorite/remove` (`{"status": "removed"}`) and `POST /api/favorite/status` (`{"is_favorite": bool}`) exist purely to support the `exists`-state toggle below; unlike watch-later, favoriting has no join table, so `remove`/`status` need no new `webapp/db/videos.py` functions — `remove` calls the same `set_favorite(..., False)` used by `add`, and `status` reads `is_favorite` straight off the `get_video_by_id` row.

Mirror-image behavior for the `exists` state: the popup's `exists` state renders two disabled checkboxes side by side, each enabled independently once its own status fetch resolves — "Add to Watch Later" (`initWatchLaterToggle`, `POST /api/watch-later/status` → `in_queue`) and "Mark as favorite (★)" (`initFavoriteToggle`, `POST /api/favorite/status` → `is_favorite`). Both follow the identical shape: `change` fires `add`/`remove` directly (no separate "apply" step, unlike the add-time checkboxes which are read at click time by `doAdd`), reverts the checkbox and shows an inline error (`#wl-error` / `#fav-error`) on failure, and re-enables regardless of outcome. `initFavoriteToggle`'s success check is narrower than watch-later's (`data.status === 'added'`/`'removed'` only, no `already_in_queue`-style alternate-success value, since the favorite API has none).
